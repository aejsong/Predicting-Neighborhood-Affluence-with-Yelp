...
def affluency_predictor(to_predict):
    ...
    predict_array = df[df['zip_code'] == Z_cluster.pivot_table(index='zip_code')[Z_cluster.pivot_table(index='zip_code').index == 90012].values
    print(predict_array)
    # load trained model
    loaded_model = pickle.load(open("model_j.p", "rb"))
    # predict
